...
public class UnweightedGraph<V> implements Graph<V> {
	...
	/*
	 * Algorithm for Exercise 28-3 public SearchTree dfsNonRecurs(int v) { int[]
	 * parent = new int[vertices.size()]; for (int i = 0; i < parent.length; i++)
	 * parent[i] = -1; // Initialize parent[i] to -1 boolean[] isVisited = new
	 * boolean[vertices.size()]; List<Integer> searchOrder = new ArrayList<>();
	 * Stack<Integer> stack = new Stack<>();
	 *
	 * push v to the stack add v to search order mark v visited
	 *
	 * while (the stack is not empty) { peek a vertex from the stack, call x if
	 * (neighbors list for x equals size 0) { pop a vertex from the stack } else {
	 * for (int i = all the vertices in x's neighbor list) { grab Egde at index i,
	 * call e remove ending vertex from x's neighbor list
	 * 
	 * if (ending vertex of e is not visited) { mark parent of ending vertex as x
	 * push ending vertex onto stack mark ending vertex as visited add ending vertex
	 * to search order break; } } }
	 *
