...
//	 create a list with all the vertices
		List<V> verticesA = getVertices();
		ArrayList<Integer> vertices = new ArrayList<>();
		for (V vert : verticesA) {
			vertices.add(getIndex(vert));
		}
...
//	  while (there are unvisited vertices) { // check list size for this
		while (vertices.size() != 0) {
			List<Integer> neighbors = null; // NOTE: This needs to be declared out here so it doesn't reset every time
//		 pick one, say v;
			int v = vertices.get(vertices.size() - 1);
			// NOTE: everything deals with its index, not the element itself.
//		 create stack;
...
//				peek a vertex from the stack, say x; 
				...
				if (this.neighbors.get(x).size() == 0) {
//					pop a vertex from the stack;
					stack.pop();
				} else {

//					loop through all neighbors of x with loop variable i {
					for (int i = 0; i < this.neighbors.get(x).size(); i++) {
//					for(int i : neighbors) {
//						Let e be the edge between x and the neighbor;
						Edge e = this.neighbors.get(x).get(i);
//						int ev = neighbors.get(i); // this should be e.v
...
//							remove e.v from list with all vertices;
							vertices.remove((Object) e.v); // NOTE: verticies are all V not int so we get the value of
															// the index
//							remove loop variable i from x's neighbor list;
							this.neighbors.get(x).remove((Object) e.v); // NOTE: this might be screwing up the list as
																		// it reduces the
							// size every iteration
						}
						// else if (e.v is not x's parent) {
						else if (parents[x] != e.v) {
...
//							remove loop variable i from list with neighbors;
							// NOTE: I'm not sure exactly what this means.
							this.neighbors.get(x).remove(i);
						}
